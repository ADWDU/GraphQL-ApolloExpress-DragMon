...
class UserClass {
    ...
    static async signUp(login, password) {
        ...
        if (candidate) {
            throw new UserInputError('User with that login already exist');
        }
    ...
    static async signIn(login, password) {
        ...
        if (!user) {
            throw new UserInputError('There is no user with that login ');
        }

        const isMatch = await bcrypt.compare(password, user.password);

        if (!isMatch) {
            throw new UserInputError('Password is incorrect, please try another one');
        }
    ...
    static async purchaseCards(userId, cardsNumber) {
        ...
        if (user.balance < cardsNumber * config.get('cardCost')) {
            throw new UserInputError('User doesn`t have enough credits to purchase card.');
        }
    ...
    static async getState(userId) {
        ...
        if (!user) {
            throw new UserInputError('User is not found by id');
        }
    ...
    static async openCell(userId, cardId, cellIndex) {
        ...
        if (user.activeCard && user.activeCard.toString() !== cardId) {
            throw new UserInputError('This is not a current card of user');
        }
