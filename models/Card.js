...
cardSchema.pre('validate', function(next) {
    if (this.cells.length !== 9) {
        throw Error('There should be 9 cells in card');
    }

    if (this.crossLine.length !== 3) {
        throw new Error('There should be 3 cells on cross line');
    }
...
class CardClass {
    ...
    static async openCell(userId, cardId, cellIndex) {
        ...
        if (!card) {
            throw new UserInputError('Card doest`t exist');
        }

        if (card.userId.toString() !== userId) {
            throw new UserInputError('This card is not belonged to you');
        }

        if (card.cells[cellIndex].opened) {
            throw new UserInputError('This cell already opened');
        }
    ...
    static async endCard(endGameSession, cardId, userId) {
        ...
        if (card.played) {
            throw new UserInputError('This card already ended');
        }
